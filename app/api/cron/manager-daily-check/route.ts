...
async function ensurePolartech() {
  ...
  if (!company) {
    ...
    const insert = await supabaseRest("proar_companies?on_conflict=id", {
      method: "POST",
  ...
  const userUpsert = await supabaseRest("proar_trial_users?on_conflict=company_id,username", {
    method: "POST",
...
export async function GET(request: NextRequest) {
  if (!isCronAuthorized(request)) return NextResponse.json({ error: "Cron não autorizado." }, { status: 401 });
  ...
  try {
    ...
    for (const company of companies) {
      ...
      await supabaseRest(`proar_companies?id=eq.${encodeURIComponent(company.id)}`, {
        method: "PATCH",
    ...
    await supabaseRest("proar_manager_audit", {
      method: "POST",
      headers: { Prefer: "return=minimal" },
      body: JSON.stringify({ company_id: polartech.id, action: "DAILY_COMPANY_ACCESS_CHECK", actor: "system", created_at: now.toISOString() }),
    });
