...
export async function GET(request: NextRequest) {
  const rateLimitResponse = enforceRateLimit(request, "public-service-order", 30);
...
export async function PUT(request: NextRequest) {
  ...
  const id = snapshotId(body.companyId, body.orderId);
  ...
  const save = await fetch(`${url}/rest/v1/proar_state?on_conflict=id`, { method: "POST", headers: { ...headers(key), Prefer: "resolution=merge-duplicates,return=minimal" }, body: JSON.stringify({ id, payload }) });
