...
async function adminContext(request: NextRequest) {
  ...
  if (session.role !== "Administrador") return { error: NextResponse.json({ error: "Apenas administradores podem configurar a IA." }, { status: 403 }) };
...
export async function GET(request: NextRequest) {
  const context=await adminContext(request); if("error" in context)return context.error;
...
export async function PUT(request: NextRequest) {
  ...
  const now=new Date().toISOString(); const ok=await save(context.companyId,{provider:"openai",encrypted_api_key:encryptTenantSecret(apiKey),key_last4:apiKey.slice(-4),enabled:true,created_at:now,updated_at:now,updated_by:context.session.nome||context.session.email});
...
export async function DELETE(request: NextRequest) {
  ...
  const ok=await save(context.companyId,{provider:"openai",encrypted_api_key:"",key_last4:current?.key_last4||null,enabled:false,created_at:now,updated_at:now,updated_by:context.session.nome||context.session.email});
