...
<nav>{statuses.map(status => <span key={status}><i style={{background:statusColor(status)}}/>{status}</span>)}</nav><div className="public-blocks">{blocks.map(block => <section key={block}><header><b>{block === "Áreas Comuns" ? "ÁREAS COMUNS" : `QUADRA ${block}`}</b><span>{map.houses.filter(house => house.block === block && normalizeStatus(house.status) === "SERVIÇO CONCLUÍDO").length}/{map.houses.filter(house => house.block === block).length} concluídas</span></header><div>{map.houses.filter(house => house.block === block).map(house => <article key={house.id} style={{"--status":statusColor(house.status)} as React.CSSProperties} onDoubleClick={() => setSelected(house)}><span>{house.kind === "common" ? <Building2 size={17}/> : <House size={17}/>}</span><h2>{house.kind === "common" ? house.name : `Casa ${String(house.lot).padStart(2,"0")}`}</h2><p><i/>{normalizeStatus(house.status)}</p>
