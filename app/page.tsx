...
function OrderDetail({ order, close, onUpdate, canEdit, customers, structures, equipment }: { order: ServiceOrder; close: () => void; onUpdate: (order: ServiceOrder) => Promise<unknown>; canEdit: boolean; customers: Customer[]; structures: ModuleRecord[]; equipment: ModuleRecord[] }) {
  ...
  return <div className="modal-layer" role="dialog" aria-modal="true" aria-label={`Ordem ${order.id}`}>
    ...
    <div className="modal order-detail-modal workspace-only">
      <ServiceOrderWorkspace
        order={currentOrder}
        customers={customers as unknown as Record<string, unknown>[]}
        structures={structures as unknown as Record<string, unknown>[]}
        equipment={equipment as unknown as Record<string, unknown>[]}
        canEdit={canEdit}
        onClose={close}
        onSave={async next => {
          const saved = await onUpdate(next as ServiceOrder);
          setCurrentOrder(saved as ServiceOrder);
          return saved;
        }}
      />
    </div>
...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    {selectedOrder && <OrderDetail order={selectedOrder} customers={customerRecords} structures={moduleRecords["Unidades e setores"] ?? []} equipment={moduleRecords["Equipamentos"] ?? []} close={() => setSelectedOrder(null)} onUpdate={updateServiceOrder} canEdit={hasAction("Ordens de serviço","Editar")}/>}{/* detalhe da OS */}
  </div>;
}
