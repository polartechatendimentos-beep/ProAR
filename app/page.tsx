...
type HouseWorkItem = { id: string; block: string; lot: number; status: HouseWorkStatus | LegacyHouseWorkStatus; photo?: string; photos?: string[] | HouseStagePhoto[]; note?: string; updatedAt?: string; history: HouseWorkUpdate[] };
...
function BudgetPDV({ customers, catalog, budgets, onSave, onConvert, onDelete }: { customers: Customer[]; catalog: ModuleRecord[]; budgets: ModuleRecord[]; onSave: (record: ModuleRecord) => void; onConvert: (record: ModuleRecord, target: "Pedido" | "Ordem de serviço") => void; onDelete: (record: ModuleRecord) => void }) {
...
function HousesWorkModule({ companyId, responsibleUser = "Utilizador do ProAR" }: { companyId: string; responsibleUser?: string }) {
  ...
  const refreshWorkMap = async () => {
    ...
    try { const response = await fetch(`/api/public-work-map?company=${encodeURIComponent(companyId)}&refresh=${Date.now()}`, { cache:"no-store" }); const result = await response.json(); if (!response.ok || !result.map?.houses?.length) throw new Error(); const shared = result.map.houses as HouseWorkItem[]; setHouses(shared); localStorage.setItem(storageKey,JSON.stringify(shared)); if(result.map.token){setShareToken(result.map.token);localStorage.setItem(shareKey,result.map.token);} setReportNotice("Mapa, progresso e histórico atualizados pelo banco principal."); }
  ...
  const openUpdate = (house: HouseWorkItem) => { setEditing(house); setNextStatus(normalizeHouseStatus(house.status)); setNote(house.note ?? ""); setPhotos({}); };
  ...
  const visible = houses.filter(house => (blockFilter === "Todas" || house.block === blockFilter) && (statusFilter === "Todos" || normalizeHouseStatus(house.status) === statusFilter) && `${house.block} ${house.lot} ${house.id}`.toLowerCase().includes(query.toLowerCase()));
  ...
  return <section className="houses-app">
    ...
    <div className="houses-kpis"><article><span><House size={18}/></span><div><small>TOTAL CADASTRADO</small><strong>{houses.length}</strong><em>11 quadras</em></div></article>{HOUSE_STATUSES.map(status => { const total = houses.filter(house => normalizeHouseStatus(house.status) === status.name).length; return <article key={status.name}><i style={{background:status.color}}/><div><small>{status.name}</small><strong>{total}</strong><em>{Math.round(total / houses.length * 100)}% da obra</em></div></article>; })}</div>
...
export default function Home() {
  ...
  const convertBudget = (budget: ModuleRecord, target: "Pedido" | "Ordem de serviço") => {
    if (target === "Pedido") {
      ...
      const order: ServiceOrder = { id:`#OS-${String(Math.max(15499,...serviceOrders.map(item => Number(item.id.replace(/\D/g,""))||0))+1).padStart(5,"0")}`, client:budget.client, unit:"Unidade principal", service:budget.purchaseItems?.map(item => item.description).join(", ") || budget.name, tech:"Não definido", date:new Date().toISOString().slice(0,10), time:"A definir", address:customer?.address || "", status:"Aberta", tone:"blue", avatar:budget.client.split(" ").map(item => item[0]).slice(0,2).join(""), catalogItems:budget.purchaseItems?.map(item => ({ id:item.productId || item.id, name:item.description, kind:item.kind || "Serviço" })) };
