...
function HousesWorkModule({ companyId }: { companyId: string }) {
  ...
  const openUpdate = (house: HouseWorkItem) => { setEditing(house); setNextStatus(house.status); setNote(house.note ?? ""); setPhotos([]); };
  ...
  const saveUpdate = () => {
    ...
    const statusChanged = nextStatus !== editing.status;
    ...
    if (!observationChanged && !statusChanged && !photos.length) { setEditing(null); return; }
  ...
  return <section className="houses-app">
    ...
    {editing && <div className="modal-layer" role="dialog" aria-modal="true"><button className="modal-backdrop" onClick={() => setEditing(null)} aria-label="Fechar"/><div className="modal house-update-modal"><div className="modal-head"><div><span>ATUALIZAÇÃO DA OBRA</span><h2>Quadra {editing.block} • Lote {String(editing.lot).padStart(2,"0")}</h2><p>Altere o status, atualize a observação ou adicione até 5 novas fotos.</p></div><button onClick={() => setEditing(null)}><X size={18}/></button></div><div className="house-update-body"><label>Novo status<select value={nextStatus} onChange={event => setNextStatus(event.target.value as HouseWorkStatus)}>{HOUSE_STATUSES.map(status => <option key={status.name}>{status.name}</option>)}</select></label><div className="status-preview" style={{"--preview-color":statusColor(nextStatus)} as React.CSSProperties}><i/><span>{nextStatus}</span></div><div className="house-photo-area"><label className={`house-photo-upload ${photos.length ? "has-photo" : ""}`}><ImageIcon size={25}/><b>{photos.length ? `${photos.length} de 5 fotos selecionadas` : "Anexar fotos da etapa"}</b><small>Câmera ou galeria • máximo de 5 fotos</small><input type="file" accept="image/*" multiple onChange={event => readPhotos(event.target.files)}/></label>{photos.length > 0 && <div className="house-photo-grid">{photos.map((photo,index) => <figure key={`${photo.slice(-20)}-${index}`}><img src={photo} alt={`Evidência ${index + 1}`}/><button type="button" aria-label={`Remover foto ${index + 1}`} onClick={() => setPhotos(current => current.filter((_,photoIndex) => photoIndex !== index))}><X size={12}/></button></figure>)}</div>}</div><label>Observação atual<textarea value={note} onChange={event => setNote(event.target.value)} placeholder="Descreva o serviço executado, pendências ou materiais utilizados..."/><small>O texto pode ser corrigido ou complementado sem alterar o status.</small></label></div><div className="modal-actions"><button className="outline-btn" onClick={() => setEditing(null)}>Cancelar</button><button className="primary-btn" onClick={saveUpdate}><CheckCircle2 size={15}/> Atualizar observação e status</button></div></div></div>}
