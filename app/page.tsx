...
function Agenda({ serviceOrders, onOpen, onSelect }: { serviceOrders: ServiceOrder[]; onOpen: (name: string) => void; onSelect: (order: ServiceOrder) => void }) {
  // Ordens canceladas permanecem no histórico, mas nunca podem ocupar a agenda operacional.
  const scheduled = [...serviceOrders].filter(order => order.date && !/^cancelad[oa]?\b/i.test(order.status.trim())).sort((a, b) => `${a.date}${a.time}`.localeCompare(`${b.date}${b.time}`));
  return <section className="module-page">
...
function SalesPDV({ customers, structures, records, sales, onSave }: { customers: Customer[]; structures: ModuleRecord[]; records: ModuleRecord[]; sales: ModuleRecord[]; onSave: (record: ModuleRecord) => void }) {
  ...
  const editSavedSale = (sale: ModuleRecord) => {
    ...
    setCart((sale.purchaseItems ?? []).map(item => ({ id:item.productId || item.id, name:item.description, code:item.productId || item.id, price:item.unitValue, quantity:item.quantity, kind:item.kind || "Produto", unit:records.find(record => record.id === (item.productId || item.id))?.unitOfMeasure || "un" })));
  ...
  return <section className={`pdv-page ${fullScreen ? "pdv-fullscreen" : ""}`} ref={pdvRef}>
    <CommercialRecordsManager title="Vendas" records={sales} onEdit={editSavedSale}/>
  ...
  const saveUpdate = async () => {
    ...
    if (missing.length) {
      const message = missing.length === 1
        ? `Adicione a foto ${missing[0]} antes de salvar esta etapa.`
        : `Adicione as fotos obrigatórias: ${missing.join(", ")}.`;
      setSaveError(message); setReportNotice(message); return;
    }
