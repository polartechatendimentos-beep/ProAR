...
type ExternalObservation = { id: string; author: string; role: string; houseId: string; environment?: string; stage?: string; description: string; photos?: string[]; createdAt: string; status: "Aberto" | "Em análise" | "Resolvido" };
...
function customerLocations(customerName: string, customers: Customer[], structures: ModuleRecord[], unitName = "") {
  ...
  const isRoom = (item: ModuleRecord) => Boolean(item.environmentType) || /sala|ambiente|consult|labor|farm|uti|recep|almox|audit|cozinha|cpd|quarto/i.test(`${item.name} ${item.category ?? ""}`);
  const isSector = (item: ModuleRecord) => /secretaria|setor|departamento|área|local de atendimento/i.test(`${item.category ?? ""} ${item.hierarchyLevel ?? ""}`) && !isRoom(item);
  const units = all.filter(item => !isSector(item) && !isRoom(item));
  const selectedUnit = all.find(item => item.name === unitName);
  const sectors = unitName ? all.filter(item => isSector(item) && (normalizeRelation(item.parentUnit) === normalizeRelation(unitName) || item.parentId === selectedUnit?.id)) : [];
  const unlinkedSectors = all.filter(item => isSector(item) && !item.parentUnit && !item.parentId);
...
function CustomerDetail({ customerName, customers, structures, serviceOrders, modules, canEdit, onBack, onOpen, onUpdateStructure }: { customerName: string; customers: Customer[]; structures: ModuleRecord[]; serviceOrders: ServiceOrder[]; modules: Record<string, ModuleRecord[]>; canEdit: boolean; onBack: () => void; onOpen: (name: string) => void; onUpdateStructure: (record: ModuleRecord) => void }) {
  ...
  return <CustomerProfileWorkspace
    ...
    onBack={onBack}
    onOpen={onOpen}
    onUpdateCustomer={updated => window.dispatchEvent(new CustomEvent("proar:update-customer", { detail: updated }))}
    onUpdateStructure={updated => onUpdateStructure(updated as ModuleRecord)}
  />;
}
...
function BiddingModule({ onOpen }: { onOpen: (tender: Tender) => void }) {
...
function HousesWorkModule({ companyId, company, responsibleUser = "Utilizador do ProAR" }: { companyId: string; company: TenantCompany; responsibleUser?: string }) {
  ...
  const publishPublicMap = async (next: HouseWorkItem[], revision = serverRevision, accessList = activeProject.externalAccess ?? []) => {
    if(!navigator.onLine){localStorage.setItem(`${shareKey}:pending`,"1");setMapOnline(false);throw new Error("Dispositivo sem internet.");}
    const response=await fetch("/api/public-work-map",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({companyId,workId:activeProject.id,workName:activeProject.name,title:`Acompanhamento da obra — ${activeProject.name}`,houses:next,externalAccess:accessList,baseRevision:revision})});
    const result=await response.json();
...
function Modal({ title, customers, structures, catalogRecords, supplierRecords, employeeRecords, close, onSave }: { title: string; customers: Customer[]; structures: ModuleRecord[]; catalogRecords: ModuleRecord[]; supplierRecords: ModuleRecord[]; employeeRecords: ModuleRecord[]; close: () => void; onSave: (data: ModalSave) => void | Promise<void> }) {
  ...
  const [priorityAttendance, setPriorityAttendance] = useState(false);
  ...
  const parentStructures = isLinkedStructure ? customerStructures(parentCustomer, customers, structures) : [];
  const parentCandidates = parentStructures.filter(item => item.id !== parentStructureId);
  ...
  return <div className="modal-layer" role="dialog" aria-modal="true" aria-label={title}><button className="modal-backdrop" onClick={close} aria-label="Fechar janela"/><div className="modal"><div className="modal-head"><div><span>{isLinkedStructure ? "ESTRUTURA DO CLIENTE • LIMITE DE 20" : "CADASTRO PROAR"}</span><h2>{isLinkedStructure ? "Nova unidade, filial ou setor" : title}</h2>{isLinkedStructure && <p>Este registro será vinculado a <strong>{parentCustomer}</strong>.</p>}</div><button onClick={close} aria-label="Fechar"><X size={18}/></button></div><div className="form-grid">
    {isLinkedStructure ? <>
      ...
      {documentLookup.message && <div className={`wide document-lookup-status ${documentLookup.kind}`} role={documentLookup.kind === "error" ? "alert" : "status"} aria-live="polite" aria-busy={documentLookup.loading}>{documentLookup.loading ? "Consultando CNPJ..." : documentLookup.message}</div>}
      ...
      <label>Tipo de ambiente<select value={environmentType} onChange={event => setEnvironmentType(event.target.value)}><option>Sala</option><option>Consultório</option><option>Recepção</option><option>Sala técnica</option><option>CPD</option><option>Laboratório</option><option>Farmácia</option><option>UTI</option><option>Outro</option></select></label>
      <label>Número da sala<input value={roomNumber} onChange={event => setRoomNumber(event.target.value)}/></label>
      <label>Andar<input value={floor} onChange={event => setFloor(event.target.value)}/></label>
      <label>Bloco<input value={block} onChange={event => setBlock(event.target.value)}/></label>
      <label>Responsável local<input value={localResponsible} onChange={event => setLocalResponsible(event.target.value)}/></label>
      <label>Ramal<input value={extension} onChange={event => setExtension(event.target.value)}/></label>
      <label>Criticidade<select value={criticality} onChange={event => setCriticality(event.target.value)}><option>Normal</option><option>Alta</option><option>Crítica</option></select></label>
      <label className="checkbox-field"><input type="checkbox" checked={priorityAttendance} onChange={event => setPriorityAttendance(event.target.checked)}/> Atendimento prioritário</label>
      ...
      {isNewOrder ? <>
        ...
        <label>Sala / Ambiente<select value={room} onChange={event => setRoom(event.target.value)} disabled={!unit}><option value="">{unit ? availableRooms.length ? "Selecione a sala (opcional)" : "Nenhuma sala cadastrada" : "Selecione uma unidade"}</option>{availableRooms.map(item => <option key={item.id} value={item.name}>{item.name} • {item.category || item.environmentType || "Ambiente"}</option>)}</select></label>
        ...
        {documentLookup.message && <div className={`wide document-lookup-status ${documentLookup.kind}`} role={documentLookup.kind === "error" ? "alert" : "status"} aria-live="polite" aria-busy={documentLookup.loading}>{documentLookup.loading ? "Consultando..." : documentLookup.message}</div>}
...
export default function Home() {
  ...
  const saveRecord = async (data: ModalSave) => {
    if (data.title.startsWith("Nova unidade, filial ou setor")) {
      ...
      const newOrder: ServiceOrder = {
        ...
        structureId: data.room ? (moduleRecords["Unidades e setores"] ?? []).find(item => item.client === data.client && item.name === data.room)?.id : undefined,
  ...
  useEffect(() => {
    const handleProfileCustomerUpdate = (event: Event) => {
      const detail = (event as CustomEvent<Customer>).detail;
      if (!detail?.id) return;
      const updated = customerRecords.map(item => item.id === detail.id ? { ...item, ...detail } : item);
      setCustomerRecords(updated);
      localStorage.setItem(companyStorageKey(activeCompany.id, "customers"), JSON.stringify(updated));
      persistSharedState(updated, serviceOrders, moduleRecords);
      setSavedMessage("Dados do cliente atualizados com sucesso.");
    };
  ...
  const createQuickCustomer = (draft: CustomerDraft): Customer | null => {
    ...
    const created: Customer = {
      id: `CLI-${Date.now().toString().slice(-7)}`,
      name: draft.name,
      legalName: draft.legalName || draft.name,
      tradeName: draft.tradeName || draft.name,
      doc: formatCnpj(draft.doc),
      contact: "",
      phone: draft.phone,
      email: draft.email,
      city: draft.city,
    ...
    const updatedCustomers = [created, ...customerRecords];
    setCustomerRecords(updatedCustomers);
    localStorage.setItem(companyStorageKey(activeCompany.id, "customers"), JSON.stringify(updatedCustomers));
    persistSharedState(updatedCustomers, serviceOrders, moduleRecords);
    return created;
  ...
  const createQuickStructure = (draft: StructureDraft & { client: string }): ModuleRecord | null => {
    const created: ModuleRecord = {
      id: `SET-${Date.now().toString().slice(-7)}`,
      name: draft.name,
      client: draft.client,
      description: draft.description,
      category: draft.kind,
      hierarchyLevel: draft.kind === "Ambiente" ? "Sala" : draft.kind,
      parentId: draft.parentId || undefined,
      environmentType: draft.kind === "Ambiente" ? draft.environmentType : undefined,
