...
function Licitacoes() {
  ...
  const [keyword, setKeyword] = useState("ar condicionado climatização manutenção");
  ...
  const visible = results.filter(item => {
    ...
    const haystack = `${item.objetoCompra || ""} ${item.orgaoEntidade?.razaoSocial || ""}`.toLowerCase();
    const matchesKeyword = !terms.length || terms.some(term => haystack.includes(term));
    const distance = distanceOf(item);
    return matchesKeyword && (!radius || distance === undefined || distance <= Number(radius));
  });
