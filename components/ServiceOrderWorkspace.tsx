...
export function ServiceOrderWorkspace({ order, customers = [], structures = [], equipment = [], canEdit, onSave, onClose }: Props) {
  ...
  const hasEditableCatalog = Array.isArray(draft.catalogItems);
  const servicesTotal = hasEditableCatalog ? catalog.filter(item => item.kind === "Serviço").reduce((sum, item) => sum + Number((item as unknown as RecordItem).price || 0), 0) : Number(draft.servicesTotal || 0);
  const productsTotal = hasEditableCatalog ? catalog.filter(item => item.kind === "Produto").reduce((sum, item) => sum + Number((item as unknown as RecordItem).price || 0), 0) : Number(draft.productsTotal || 0);
  const discount = Number(draft.discount || 0);
  const total = Math.max(0, servicesTotal + productsTotal - discount);
  const locked = /^cancelada$/i.test(String(order.status || ""));
  ...
  return <section className="os-workspace" aria-label={`Área de trabalho da ${draft.id}`}>
    <header className="os-workspace-header">
      ...
      <div className="os-header-actions"><span className={`os-status ${draft.tone || "blue"}`}><i/> {draft.status}</span><button className="os-primary-button" disabled={!editable || saving} onClick={() => void save()}><Save size={15}/> {saving ? "Salvando..." : "Salvar"}</button></div>
    </header>

    <fieldset className="os-workspace-editor" disabled={!editable}>
    ...
    {locked && <div className="os-notice os-readonly-notice"><History size={15}/> OS cancelada: consulta e impressão disponíveis, edição bloqueada.</div>}
