...
export function normalizePncpRecord(raw: unknown, collectedAt: string) {
  ...
  const normalized: ProcurementRecord = {
    ...
    plataforma: "PNCP",
    status: normalizeProcurementStatus(item.situacaoCompraNome ?? item.situacaoNome ?? item.status, item.dataEncerramentoProposta),
    statusOriginal: safeText(item.situacaoCompraNome ?? item.situacaoNome ?? item.status),
    source: "PNCP",
    sourceId: "pncp",
    sourceUrl,
    collectedAt,
    ...
    items,
    relevance: {
      matchedTerms: HVAC_TERMS.filter(term => normalizedText([title, item.informacaoComplementar, ...items].join(" ")).includes(normalizedText(term))),
      context: normalizedText([title, item.informacaoComplementar, ...items].join(" ")).includes("automot") ? "frota_automotiva_revisar" : "hvac",
      evidence: sourceUrl || PNCP_DOCS,
    },
  };
